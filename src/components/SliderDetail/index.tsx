...
const SliderDetail = ({ data = [] }: { data: string[] }) => {
    ...
    <SliderDetailWrapper>
      ...
      <div className="relative sm:px-14 w-full">
        <div className="max-w-full md:w-[400px] mt-12 relative z-10">
          ...
          >
            {data?.map((item, id) => (
              <div className="cursor-pointer capitalize relative shadow-lg p-4" key={item + id}>
        ...
        <div className="absolute top-1/2 flex justify-between w-full -translate-y-1/2">
          <PrevArrow
            className="slick-arrow slick-prev absolute top-0 -translate-y-1/2 -translate-x-8"
            onClick={prevSlide}
          />
          <NextArrow
            className="slick-arrow slick-next absolute top-0 -translate-y-1/2 -translate-x-8"
            onClick={nextSlide}
          />
        </div>
      </div>
